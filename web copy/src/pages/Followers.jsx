...
export default function Followers() {
  ...
  const loadPage = useCallback(async (p = 1) => {
    ...
    setError('')
    try {
      const { data } = await UserApi.followers(username, { page: p, limit: PAGE_LIMIT })
      ...
      setError(e?.response?.data?.message || e?.message || 'Failed to load followers')
    } finally {
      setLoading(false)
    }
  }, [username])
  ...
  if (!loading && items.length === 0) return <Empty>No followers.</Empty>
