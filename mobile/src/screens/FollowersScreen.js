...
import { getFollowers, followUser, unfollowUser } from '../services/api';
...
export default function FollowersScreen({ route, navigation }) {
  ...
  // silent refresh when returning to this screen
  useFocusEffect(
    useCallback(() => {
  ...
  const onToggleFollow = async (targetUsername, currentlyFollowing) => {
    ...
    try {
      if (currentlyFollowing) {
        ...
        setItems(prev => prev.map(u => u.username === targetUsername ? { ...u, isFollowing: false } : u));
      } else {
    ...
    <View style={{ flex: 1, backgroundColor: '#F8F4FF' }}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<View style={styles.center}><Text style={styles.muted}>No followers</Text></View>}
          ListFooterComponent={
      ...
      {!!err && <View style={styles.error}><Text style={styles.errText}>{err}</Text></View>}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  error: { position: 'absolute', bottom: 16, left: 16, right: 16, backgroundColor: '#FFE6E6', borderRadius: 8, padding: 10 },
